...
class IfYouHaveTheNameOfASpecificSoftwareProductAndWantToEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "if_you_have_the_name_of_a_specific_software_product_and_want_to";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new IfYouHaveTheNameOfASpecificSoftwareProductAndWantToEntity($this->_client, $opts);
    }

    /**
     * @param IfYouHaveTheNameOfASpecificSoftwareProductAndWantTo|array $args IfYouHaveTheNameOfASpecificSoftwareProductAndWantTo data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = CvedbHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return IfYouHaveTheNameOfASpecificSoftwareProductAndWantTo|array The current IfYouHaveTheNameOfASpecificSoftwareProductAndWantTo data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of IfYouHaveTheNameOfASpecificSoftwareProductAndWantTo fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = CvedbHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of IfYouHaveTheNameOfASpecificSoftwareProductAndWantTo fields).
     */
    ...
    /**
     * Load a single IfYouHaveTheNameOfASpecificSoftwareProductAndWantTo.
     *
     * @param IfYouHaveTheNameOfASpecificSoftwareProductAndWantToLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed IfYouHaveTheNameOfASpecificSoftwareProductAndWantToLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return IfYouHaveTheNameOfASpecificSoftwareProductAndWantTo|array The loaded IfYouHaveTheNameOfASpecificSoftwareProductAndWantTo as an assoc-array at the
     *   SDK boundary; throws CvedbError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = CvedbHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
