...
class CveEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = CvedbSDK::test(null, null);
        $ent = $testsdk->Cve(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = cve_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "cve." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CVEDB_TEST_CVE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function cve_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/cve/CveTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["cve01", "cve02", "cve03"] as $k) {
    ...
    $entid_env_raw = getenv("CVEDB_TEST_CVE_ENTID");
