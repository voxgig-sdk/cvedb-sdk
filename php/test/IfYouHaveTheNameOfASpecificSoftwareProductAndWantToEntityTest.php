...
class IfYouHaveTheNameOfASpecificSoftwareProductAndWantToEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = CvedbSDK::test(null, null);
        $ent = $testsdk->IfYouHaveTheNameOfASpecificSoftwareProductAndWantTo(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = if_you_have_the_name_of_a_specific_software_product_and_want_to_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "if_you_have_the_name_of_a_specific_software_product_and_want_to." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CVEDB_TEST_IF_YOU_HAVE_THE_NAME_OF_A_SPECIFIC_SOFTWARE_PRODUCT_AND_WANT_TO_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $if_you_have_the_name_of_a_specific_software_product_and_want_to_ref01_data = null;
        if (count($if_you_have_the_name_of_a_specific_software_product_and_want_to_ref01_data_raw) > 0) {
...
function if_you_have_the_name_of_a_specific_software_product_and_want_to_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/if_you_have_the_name_of_a_specific_software_product_and_want_to/IfYouHaveTheNameOfASpecificSoftwareProductAndWantToTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["if_you_have_the_name_of_a_specific_software_product_and_want_to01", "if_you_have_the_name_of_a_specific_software_product_and_want_to02", "if_you_have_the_name_of_a_specific_software_product_and_want_to03"] as $k) {
    ...
    $entid_env_raw = getenv("CVEDB_TEST_IF_YOU_HAVE_THE_NAME_OF_A_SPECIFIC_SOFTWARE_PRODUCT_AND_WANT_TO_ENTID");
