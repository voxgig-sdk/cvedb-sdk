...
{
    private static function utility(): CvedbUtility
    {
        return new CvedbUtility();
    }
    ...
    private static function ctx(array $over = []): CvedbContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new CvedbContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new CvedbOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof CvedbError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new CvedbOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = CvedbMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new CvedbOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = CvedbMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new CvedbOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = CvedbMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CvedbSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = CvedbMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new CvedbResponse([]),
            'result' => new CvedbResult([]),
        ]);
        [, $err] = CvedbMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new CvedbSpec(['step' => 's']),
            'response' => new CvedbResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new CvedbResult([]),
        ]);
        [, $err] = CvedbMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new CvedbSpec(['step' => 's']),
            'response' => new CvedbResponse(self::resp(200, ['v' => 1])),
            'result' => new CvedbResult([]),
        ]);
        [, $err] = CvedbMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new CvedbSpec(['step' => 's']),
            'response' => new CvedbResponse(self::resp(200, ['v' => 2])),
            'result' => new CvedbResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        CvedbMakeResponse::call($ctx);
    ...
    {
        $preset = new CvedbResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new CvedbSpec([]),
            'response' => new CvedbResponse([]),
            'result' => new CvedbResult([]),
        ...
        [$got, $err] = CvedbMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new CvedbResult([])]);
        [, $err] = CvedbMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new CvedbSpec([]), 'result' => null]);
        [, $err] = CvedbMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CvedbSpec(['step' => 's']),
            'result' => new CvedbResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new CvedbOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CvedbMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CvedbSpec(['step' => 's']),
            'result' => new CvedbResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new CvedbOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CvedbMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CvedbResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new CvedbSpec([]),
            'result' => new CvedbResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = CvedbMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $utility = self::utility();
        $boom = new CvedbError('boom', 'boom');
        $utility->fetcher = function (CvedbContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CvedbSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = CvedbMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (CvedbContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = CvedbMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(CvedbResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (CvedbContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CvedbSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = CvedbMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new CvedbResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new CvedbSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = CvedbMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = CvedbMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = CvedbMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new CvedbResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, CvedbDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new CvedbResult(['ok' => false])]);
        $this->expectException(CvedbError::class);
        CvedbDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new CvedbResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', CvedbMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new CvedbBaseFeature();
        $b = new CvedbBaseFeature();
        CvedbFeatureAdd::call($ctx, $a);
        CvedbFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): CvedbBaseFeature
    {
        $f = new CvedbBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    private static function auth_ctx(array $options, ?array $headers): CvedbContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new CvedbSpec(['headers' => $headers]),
        ]);
    }
    ...
    {
        $ctx = self::ctx([
            'response' => new CvedbResponse(['headers' => null]),
            'result' => new CvedbResult([]),
        ]);
        CvedbResultHeaders::call($ctx);
