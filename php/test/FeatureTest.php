...
class FtCtrl extends CvedbControl
{
...
class FtClient
{
    ...
    private CvedbUtility $utility;

    public function __construct(CvedbUtility $utility, array $options)
    {
    ...
    public function get_utility(): CvedbUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CvedbContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CvedbContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CvedbConfig::make_feature($name);
    ...
    private static function buildUrl(CvedbSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CvedbError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CvedbResponse($fetched);
        }
    ...
    private function fail(CvedbContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CvedbContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
