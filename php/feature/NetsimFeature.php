...
class CvedbNetsimFeature extends CvedbBaseFeature
{
    ...
    private function _simulate(CvedbContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(CvedbContext $ctx, array $applied): void
    {
        $client = $this->client;
