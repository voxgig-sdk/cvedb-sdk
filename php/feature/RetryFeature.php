...
class CvedbRetryFeature extends CvedbBaseFeature
{
    ...
    public function init(CvedbContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (CvedbContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(CvedbContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(CvedbContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
