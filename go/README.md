...
## Install
```bash
go get github.com/voxgig-sdk/cvedb-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/cvedb-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/cvedb-sdk/go=../cvedb-sdk/go
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *CvedbSDK
```
...
| `Cve` | `(data map[string]any) CvedbEntity` | Create a Cve entity instance. |
| `IfYouHaveTheNameOfASpecificSoftwareProductAndWantTo` | `(data map[string]any) CvedbEntity` | Create an IfYouHaveTheNameOfASpecificSoftwareProductAndWantTo entity instance. |
| `ThisEndpointIsTailoredForSearchesBasedOnProductNameOr` | `(data map[string]any) CvedbEntity` | Create a ThisEndpointIsTailoredForSearchesBasedOnProductNameOr entity instance. |

### Entity interface (CvedbEntity)

All entities implement the `CvedbEntity` interface.
...
Create an instance: `cve := client.Cve(nil)`
...
| --- | --- | --- |
| `cpes` | `[]any` | A list of Common Platform Enumeration (CPE) identifiers that specify the affected product(s) or component(s) by this vulnerability. |
...
| `references` | `[]any` | A list of references providing further details, technical advisories, and mitigation guidance related to the vulnerability, facilitating deeper understanding and research. |
...
```go
cve, err := client.Cve(nil).Load(map[string]any{"id": "cve_id"}, nil)
if err != nil {
    panic(err)
}
fmt.Println(cve) // the loaded record
```
...
```
github.com/voxgig-sdk/cvedb-sdk/go/
├── cvedb.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/cvedb-sdk/go`) re-exports everything needed
