...
import (
	...
	omni "github.com/voxgig-sdk/cvedb-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CvedbError); is {
				out := map[string]any{
					"name":    "CvedbError",
...
func omniCtx(arg any, client *sdk.CvedbSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CvedbSDK); is {
			client = live
