...
[MCP](https://modelcontextprotocol.io) server exposing the Cvedb SDK as
two agent tools — `cvedb_list` and `cvedb_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/cvedb-mcp)
make build

# 2. Provide credentials via the environment
export CVEDB_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user cvedb \
  -- /absolute/path/to/cvedb-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./cvedb-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// cvedb_list: first page of records
{ "entity": "cve" }
{ "entity": "cve", "query": { } }

// cvedb_load: one record by id
{ "entity": "cve", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/cvedb-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user cvedb \
     -- "$PWD"/dist/*/cvedb-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `cvedb_list` and `cvedb_load` tools now appear
   in new sessions. Ask the agent to *"list cve using cvedb"*
   and it calls `cvedb_list` with `{"entity":"cve"}`.
...
```sh
export CVEDB_APIKEY=sk_live_xxx            # API key
export CVEDB_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `cvedb_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `cvedb_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CVEDB_APIKEY` | API key sent with every request. |
| `CVEDB_BASE` | Optional override of the API base URL. |
...
```sh
./cvedb-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"cvedb_load","arguments":{"entity":"cve","query":{"id":1}}}}'
```
