...
The Lua SDK for the Cvedb API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Cve(nil):load({ id = "example_id" }, nil)
...
local result, err = client:Cvedb(nil):load(
  { id = "test01" }, nil
)
