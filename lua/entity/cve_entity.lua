...
local CveEntity = {}
CveEntity.__index = CveEntity


function CveEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CveEntity)
  self._name = "cve"
  self._client = client
...
function CveEntity:get_name()
  return self._name
end


function CveEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CveEntity.new(self._client, opts)
end
...
function CveEntity:mark_deleted()
  self._deleted = true
end


function CveEntity:deleted()
  return true == self._deleted
end


function CveEntity:data_set(args)
  if args ~= nil then
...
function CveEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CveEntity:match_set(args)
  if args ~= nil then
...
function CveEntity:stream(action, args, callopts)
...
function CveEntity:_run_op(ctx, post_done)
  local utility = self._utility
