...
//    only - a typed CvedbContext there would make every ctx
...
final class CvedbOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new CvedbContext($clean, null);
    ...
    public static function err_from_map($m): ?CvedbError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new CvedbError(is_string($code) ? $code : '', $msg);
    }
