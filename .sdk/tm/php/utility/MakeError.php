...
class CvedbMakeError
{
    public static function call(?CvedbContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new CvedbContext([], null);
        }
        $op = $ctx->op ?? new CvedbOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new CvedbResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof CvedbError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new CvedbError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof CvedbError) {
