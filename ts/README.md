...
`client.Cve()` — each with a small set of operations (`load`)
...
```ts
try {
  const ifyouhavethenameofaspecificsoftwareproductandwantto = await client.IfYouHaveTheNameOfASpecificSoftwareProductAndWantTo().load({ product: "example" })
...
await entity.load({ product: 'example_product' })
...
```ts
const if_you_have_the_name_of_a_specific_software_product_and_want_to = await client.IfYouHaveTheNameOfASpecificSoftwareProductAndWantTo().load({ product: 'product' })
```
...
```ts
const this_endpoint_is_tailored_for_searches_based_on_product_name_or = await client.ThisEndpointIsTailoredForSearchesBasedOnProductNameOr().load()
```
...
await ifyouhavethenameofaspecificsoftwareproductandwantto.load({ product: "example" })
