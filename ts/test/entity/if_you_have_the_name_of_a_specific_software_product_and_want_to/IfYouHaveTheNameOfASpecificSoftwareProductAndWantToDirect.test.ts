...
describe('IfYouHaveTheNameOfASpecificSoftwareProductAndWantToDirect', async () => {
  ...
  test('direct-load-if_you_have_the_name_of_a_specific_software_product_and_want_to', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-if_you_have_the_name_of_a_specific_software_product_and_want_to', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CVEDB_TEST_IF_YOU_HAVE_THE_NAME_OF_A_SPECIFIC_SOFTWARE_PRODUCT_AND_WANT_TO_ENTID']
