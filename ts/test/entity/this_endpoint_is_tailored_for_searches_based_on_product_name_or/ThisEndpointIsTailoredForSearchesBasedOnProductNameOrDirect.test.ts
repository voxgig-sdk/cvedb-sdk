...
describe('ThisEndpointIsTailoredForSearchesBasedOnProductNameOrDirect', async () => {
  ...
  test('direct-load-this_endpoint_is_tailored_for_searches_based_on_product_name_or', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-this_endpoint_is_tailored_for_searches_based_on_product_name_or', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CVEDB_TEST_THIS_ENDPOINT_IS_TAILORED_FOR_SEARCHES_BASED_ON_PRODUCT_NAME_OR_ENTID']
