...
//     (\`new CvedbSDK(...)\` / \`CvedbSDK.test(...)\` become
//     \`CvedbSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"cve":{"test01":{"id":"test01"}},"if_you_have_the_name_of_a_specific_software_product_and_want_to":{"test01":{"id":"test01"}},"this_endpoint_is_tailored_for_searches_based_on_product_name_or":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new CvedbSDK(...)\` and any
// \`CvedbSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CvedbSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CvedbSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
