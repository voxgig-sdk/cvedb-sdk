# Cvedb SDK test runner

from __future__ import annotations
import os
import json

from cvedb_sdk.utility.voxgig_struct import voxgig_struct as vs


class CvedbTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CvedbTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CvedbTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CvedbTestRunner.getenv("CVEDB_TEST_LIVE")
        override = CvedbTestRunner.getenv("CVEDB_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CvedbTestRunner.getenv(key)
        ...
        explain = CvedbTestRunner.getenv("CVEDB_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["CVEDB_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if CvedbTestRunner._test_control is not None:
            return CvedbTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CvedbTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CvedbTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CvedbTestRunner.load_test_control()
...
def load_env_local():
    CvedbTestRunner.load_env_local()


def env_override(m):
    return CvedbTestRunner.env_override(m)


def entity_data(v):
    return CvedbTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CvedbTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CvedbTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CvedbTestRunner.load_test_control()


def live_client_options():
    return CvedbTestRunner.live_client_options()


def live_delay_ms():
    return CvedbTestRunner.live_delay_ms()
