...
The Python SDK for the Cvedb API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Cve(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.Cvedb(None).load(
    {"id": "test01"}, None
)
