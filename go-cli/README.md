...
boru-driven command-line client **and** interactive REPL for the Cvedb
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/cvedb-cli)
make build
...
./cvedb-cli load 1 cve            # {id:1} shorthand
./cvedb-cli load '{id:1}' cve       # explicit match map

# 5. Override the API base URL for a single call
CVEDB_BASE=https://api.example.com ./cvedb-cli load 1 cve
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/cvedb-cli
   ```
...
```sh
./cvedb-cli load 1 cve          # scalar shorthand for {id:1}
./cvedb-cli load '{id:1}' cve     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `cvedb>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `cve`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/cvedb-cli`. |
...
(`./cvedb-cli <expr>`) and interactively (the REPL), and expressions compose the
