// Command Cvedb-cli is a CLI + REPL for the Cvedb SDK,
...
const entitiesHelp = "cve if_you_have_the_name_of_a_specific_software_product_and_want_to this_endpoint_is_tailored_for_searches_based_on_product_name_or"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CVEDB_APIKEY (api key), CVEDB_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CVEDB_APIKEY carries the API key and
	// CVEDB_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CVEDB_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CVEDB_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCvedbSDK(opts)
